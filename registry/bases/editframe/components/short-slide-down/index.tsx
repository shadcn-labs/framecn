...
export const ShortSlideDown = ({
  ...
  return (
    ...
    >
      <>
        <style>{`
          @keyframes framecn-ssd-slide {
            from { transform: translateY(-${distance}px); }
            to { transform: translateY(0); }
          }
          @keyframes framecn-ssd-fade {
            from { opacity: 0; }
            to { opacity: 1; }
          }
        `}</style>
        <span
          style={{
            animation: `framecn-ssd-slide ${containerMs}ms cubic-bezier(0.2, 0.8, 0.2, 1) 0ms backwards`,
            color,
        ...
        >
          {words.map((word, i) => (
            <span
              key={i}
              style={{
                animation: `framecn-ssd-fade ${fadeMs}ms ease-out ${i * staggerMs}ms backwards`,
                display: "inline-block",
                marginRight: "0.25em",
              }}
            >
